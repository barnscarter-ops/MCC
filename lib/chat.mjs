...
import { callClaude, callPiRpc, openRouterChat, localChat } from './models.mjs';
...
async function delegateWriteToExecutor(task, staged, controller) {
  ...
  // Local llama.cpp (custom Qwen) is the primary executor; OpenRouter GLM is the
  // fallback when the local server is offline so OPS never goes dark.
  const editorOpts = { signal: controller.signal, temperature: 0.1, maxTokens: 4000 };
  let newContent;
  try {
    newContent = await localChat(editorMessages, editorOpts);
  } catch (err) {
    if (err.name === 'AbortError') throw err;
    console.warn(`[executor] local model failed — OpenRouter fallback: ${err.message}`);
