...
function buildEstimateSummary(extracted) {
  ...
  if (!items.length) {
    ...
    for (const item of items) {
      ...
      md += `| ${badge} | ${item.name} | ${item.quantity} | ${price} |\n`;
    }
...
export async function handleChat(req, res) {
  try {
    const { prompt, mode = 'ask', history = [], attachments = [], lineItems, pendingItems, pendingCustomer } = await readJsonBody(req);
    ...
    // ESTIMATE-READY → pre-structured line items from ASK MAVERICK, bypass extraction
