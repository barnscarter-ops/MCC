...
|------|---------|-------------|
| `ask` | RAG → Codex/Qwen fallback | General Q&A + estimate intent detection |
| `build` | Codex director → Qwen executor → NIM QC | Agentic coding loop |
| `ops` | Codex orchestrator → ops tools | Email, docs, spreadsheets, agents |
| `Codex` | Codex CLI session | Full filesystem access via Superpowers |
...
- All three repos must be selected at Codex web session creation for MCC to be clonable
